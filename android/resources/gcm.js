...
(function (service) {
	...
	title = serviceIntent.hasExtra('title') ? serviceIntent.getStringExtra('title') : Ti.App.name,
	statusBarMessage = serviceIntent.hasExtra('msg') ? decodeURIComponent((serviceIntent.getStringExtra('msg')+'').replace(/\+/g, '%20')) : '',
	message = serviceIntent.hasExtra('msg') ? decodeURIComponent((serviceIntent.getStringExtra('msg')+'').replace(/\+/g, '%20')) : '',
	...
	launcherIntent.putExtra("msg.notificationId", serviceIntent.getStringExtra('msg.notificationId'));

	// increase notification id
	ntfId += 1;
	Ti.App.Properties.setInt('ntfId', ntfId);
	Ti.App.Properties.setString('msg.notificationId', serviceIntent.getStringExtra('msg.notificationId'));
	Ti.App.Properties.setString('msg.segmentId', serviceIntent.getStringExtra('msg.segmentId'));
